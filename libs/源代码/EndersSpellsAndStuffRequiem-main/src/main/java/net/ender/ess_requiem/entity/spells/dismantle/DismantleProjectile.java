...
public class DismantleProjectile extends Projectile implements AntiMagicSusceptible {
    private List<Entity> entities;
    private static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(DismantleProjectile.class, EntityDataSerializers.FLOAT);
    ...
    public DismantleProjectile(EntityType<? extends Projectile> entityType, Level level) {
    ...
    public DismantleProjectile(EntityType<? extends Projectile> entityType, Level level, LivingEntity shooter)
    {
    ...
    {
        this(GGEntityRegistry.DISMANTLE.get(), level, shooter);
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            ...
            for (Entity entity : level().getEntities(this, this.getBoundingBox()).stream().filter(target -> canHitEntity(target) && !victims.contains(target)).collect(Collectors.toSet())) {
                ...
                MagicManager.spawnParticles(level(), ParticleTypes.DRIPPING_OBSIDIAN_TEAR, entity.getX(), entity.getY(), entity.getZ(), 50, 0, 0, 0, .5, true);
    ...
    private void damageEntity(Entity entity) {
        if (!victims.contains(entity)) {
            DamageSources.applyDamage(entity, damage, GGSpellRegistry.SLASH.get().getDamageSource(this, getOwner()));
            victims.add(entity);
        }
    }
    ...
    public void spawnParticles() {
        if (level().isClientSide) {
            ...
            for (int i = 0; i < width / step; i++) {
                ...
                level().addParticle(GGParticleRegistry.BIG_SLASH_LEFT.get(), false, x + rotX + dx, y + dy, z + rotZ + dz, dx, dy, dz);
                level().addParticle(GGParticleRegistry.BIG_SLASH_RIGHT.get(), false, x + rotX + dx, y + dy, z + rotZ + dz, dx, dy, dz);
            }
        }
    }
